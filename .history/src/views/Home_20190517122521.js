...
class Home extends React.Component {
  ...
  async submit(){
    try {		
      // console.log('login')
    var odoo = new Odoo({
      host: 'https://beyonderp.beyondjakarta.com',
      port: 443,
      database: 'beyond',
    });
    ...
    var params = {
      db: 'beyond',
      username: 'admin',
      password: 'pass4beyond',
    };
    var json = JSON.stringify({ params: params });
    var url = 'https://' + 'beyonderp.beyondjakarta.com' + ':' + '443' + '/web/session/authenticate';
    var options = {
      method: 'POST',
      headers: {
      ...
      result = await response1.result
      var session = 'session_id=' + result.session_id;
      ...
      // }
        ...
        error = await response1.error
        console.log('res',response1.error)
        this.setState({
          error: error.message
        })
      }
    ...
    return (
      <Container fluid className="main-content-container px-4 bg-white">
        ...
        <Row noGutters className="page-header py-4">
          <PageTitle sm="4" title="Today Recommendation" subtitle="" className="text-sm-left text-left" />
        </Row>
        {/* Recommendation */}
        <Row>
